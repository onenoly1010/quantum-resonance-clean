...
from src.schemas.schemas import AllocationConfig, AllocationRuleCreate
...
def sample_accounts(db_session):
    ...
    return {"source": source_account, "dest1": dest_account_1, "dest2": dest_account_2}
...
def test_create_allocation_rule(db_session, sample_accounts):
    ...
    allocation_config = [
        AllocationConfig(
            destination_account_id=dest1.id, percentage=Decimal("70"), priority=1
        ),
        AllocationConfig(
            destination_account_id=dest2.id, percentage=Decimal("30"), priority=2
        ),
    ]
...
def test_create_allocation_rule_invalid_source(db_session):
    ...
    allocation_config = [
        AllocationConfig(
            destination_account_id=fake_dest_id, percentage=Decimal("100"), priority=1
        )
    ]
...
def test_execute_allocation(db_session, sample_accounts):
    ...
    allocation_config = [
        AllocationConfig(
            destination_account_id=dest1.id, percentage=Decimal("60"), priority=1
        ),
        AllocationConfig(
            destination_account_id=dest2.id, percentage=Decimal("40"), priority=2
        ),
    ]
...
def test_execute_allocation_inactive_rule(db_session, sample_accounts):
    ...
    allocation_config = [
        AllocationConfig(
            destination_account_id=dest1.id, percentage=Decimal("100"), priority=1
        )
    ]
    ...
    with pytest.raises(ValueError) as exc_info:
        AllocationService.execute_allocation(db_session, rule.id, Decimal("100.00"))
