...
def list_accounts(
    ...
    accounts = (
        query.order_by(LogicalAccount.account_name).offset(skip).limit(limit).all()
    )
...
def create_account(
    ...
    existing_account = (
        ...
        .filter(LogicalAccount.account_name == account.account_name)
        .first()
    )
...
def get_account(
    ...
    Requires authentication.
    """
    account = db.query(LogicalAccount).filter(LogicalAccount.id == account_id).first()
...
def update_account(
    ...
    """
    account_record = (
        db.query(LogicalAccount).filter(LogicalAccount.id == account_id).first()
    )
...
def get_account_balance(
    ...
    last_transaction = (
        ...
        .filter(LedgerTransaction.account_id == account_id)
