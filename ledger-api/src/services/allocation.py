...
class AllocationService:
    ...
    def validate_allocation_config(allocation_config: List[Dict]) -> bool:
        ...
        total_percentage = sum(
            Decimal(str(config.get("percentage", 0))) for config in allocation_config
        )
    ...
    def create_allocation_rule(
        db: Session, rule_data: AllocationRuleCreate
    ) -> AllocationRule:
        """
        ...
        allocation_config_dicts = [
            config.model_dump() for config in rule_data.allocation_config
        ]
        AllocationService.validate_allocation_config(allocation_config_dicts)

        # Check if source account exists
        source_account = (
            db.query(LogicalAccount)
            .filter(LogicalAccount.id == rule_data.source_account_id)
            .first()
        )
        ...
        for config in rule_data.allocation_config:
            destination_account = (
                db.query(LogicalAccount)
                .filter(LogicalAccount.id == config.destination_account_id)
                .first()
            )
    ...
    def update_allocation_rule(
        db: Session, rule_id: UUID, rule_data: AllocationRuleUpdate
    ) -> AllocationRule:
        """
        ...
        """
        allocation_rule = (
            db.query(AllocationRule).filter(AllocationRule.id == rule_id).first()
        )
    ...
    def execute_allocation(
        db: Session, rule_id: UUID, amount: Decimal, reference_id: Optional[str] = None
        ...
        allocation_rule = (
            ...
            .filter(AllocationRule.id == rule_id, AllocationRule.is_active == True)
            .first()
        )
        ...
        sorted_allocations = sorted(
            allocation_rule.allocation_config, key=lambda x: x.get("priority", 999)
        )
        ...
        for config in sorted_allocations:
            allocation_amount = (amount * Decimal(str(config["percentage"]))) / Decimal(
                "100"
            )
