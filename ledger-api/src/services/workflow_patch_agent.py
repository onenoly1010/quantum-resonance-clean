...
from src.schemas.schemas import (
    WorkflowPatchCreate, WorkflowPatchUpdate, WorkflowAnalysisCreate,
    PatchDeploymentReport, WorkflowHealthReport, AnalysisFindings
)
...
class WorkflowPatchAgent:
    ...
    def analyze_workflow(
        ...
        logger.info(f"Analysis complete. Severity: {severity}, Issues: {len(findings.get('issues', []))}")
        return analysis
    
    def _perform_analysis(self, workflow_name: str, analysis_type: str) -> Dict[str, Any]:
        ...
        if analysis_type == "security":
            ...
            findings["inefficiencies"].extend(self._check_performance_issues(workflow_name))
        elif analysis_type == "efficiency":
            findings["inefficiencies"].extend(self._check_efficiency_issues(workflow_name))
    ...
    def _calculate_severity(self, findings: Dict[str, Any]) -> str:
        ...
        critical_count = sum(
            1 for issue in findings.get("issues", [])
    ...
    def create_patch(
        ...
        deployment_config = patch_data.deployment_config or self._create_default_deployment_config()
        rollback_config = patch_data.rollback_config or self._create_rollback_config(patch_content_dict)
    ...
    def test_patch(self, patch_id: UUID) -> Dict[str, Any]:
        ...
        patch = self.db.query(WorkflowPatch).filter(
            WorkflowPatch.id == patch_id
        ).first()
    ...
    def deploy_patch(
        ...
        if patch.status not in ["tested", "approved"]:
            raise ValueError(f"Patch must be tested before deployment. Current status: {patch.status}")
    ...
    def _generate_impact_report(
        ...
        return {
            ...
            "deployment_duration_seconds": deployment_result.get("total_duration_seconds", 0),
            ...
            "metrics_before": patch.test_results.get("metrics_before", {}) if patch.test_results else {},
    ...
    def get_workflow_health(self, workflow_name: str) -> WorkflowHealthReport:
        ...
        analyses = self.db.query(WorkflowAnalysis).filter(
            WorkflowAnalysis.workflow_name == workflow_name
        ).order_by(WorkflowAnalysis.created_at.desc()).limit(10).all()
        
        # Get patches
        patches = self.db.query(WorkflowPatch).filter(
            WorkflowPatch.target_workflow == workflow_name
        ).all()
        ...
        return WorkflowHealthReport(
            ...
            issues_identified=sum(len(a.findings.get("issues", [])) for a in analyses),
            patches_pending=sum(1 for p in patches if p.status in ["pending", "testing", "tested"]),
            patches_deployed=sum(1 for p in patches if p.status == "deployed"),
    ...
    def _calculate_health_score(
        ...
        deployed_patches = [p for p in patches if p.status == "deployed"]
