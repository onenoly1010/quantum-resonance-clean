...
class LogicalAccountUpdate(BaseModel):
    ...
    account_type: Optional[str] = Field(
        None, pattern="^(asset|liability|equity|revenue|expense)$"
    )
    description: Optional[str] = None
...
class WorkflowPatchBase(BaseModel):
    ...
    patch_type: str = Field(
        ..., pattern="^(bug_fix|performance|security|feature|refactor)$"
    )
    description: str
...
class WorkflowPatchUpdate(BaseModel):
    ...
    status: Optional[str] = Field(
        None, pattern="^(pending|testing|tested|approved|deployed|failed|rolled_back)$"
    )
...
class WorkflowAnalysisBase(BaseModel):
    ...
    analysis_type: str = Field(
        ..., pattern="^(security|performance|efficiency|compatibility|quality)$"
    )
